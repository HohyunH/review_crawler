...
class DB_model:
    ...
    def set_data_body(self, channel_type, row):
        ...
        if last_data['cnt'] < 1:
            ...
            c.execute((
                ...
                "`data_dislike_count`, `data_body`, `data_user_follow`, `data_user_follower`, TBL_DATA_LIST`data_user_medias`, "
    ...
    # Utility function : data diff between two days of powder_room
    def conv_date_pdr(self, t):
        if t.find("전") == -1:
            t = re.sub("\.", "-", t)
            d2 = datetime.strptime(t, "%Y-%m-%d")
            return d2.strftime("%Y-%m-%d %H:%M:%S")
